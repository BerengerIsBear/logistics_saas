...
async function getLatestJobNumber(companyId: string) {
  ...
  if (latestErr) return { ok: false as const, error: latestErr.message, latest: null as any };
...
export async function GET(req: Request) {
  ...
  // ✅ Stable search: only fields in jobs table
  // (Avoid OR on embedded join fields which is flaky in PostgREST)
  if (q) {
    const safe = q.replace(/[%_,]/g, " ").trim();
    // Search across job_number, snapshot customer, pickup, dropoff
    query = query.or(
      `job_number.ilike.%${safe}%,customer.ilike.%${safe}%,pickup.ilike.%${safe}%,dropoff.ilike.%${safe}%`
    );
  }
...
// ✅ POST create job (customer_id required)
export async function POST(req: Request) {
  ...
  // Generate job_number from latest (simple, with one retry to reduce collisions)
  const latestRes = await getLatestJobNumber(companyId);
  if (!latestRes.ok) return NextResponse.json({ error: latestRes.error }, { status: 500 });

  let jobNumber = nextJobNumberFromLatest(latestRes.latest?.job_number);

  // Try insert (and retry once if conflict)
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!error && row) {
      ...
      try {
        const { error: actErr } = await admin.from("activity").insert({
          ...
          meta: { job_number: row.job_number, customer_id: row.customer_id },
        });
    ...
    // If failed and we have another attempt, regenerate job number from latest again
    if (attempt === 0) {
      const latestAgain = await getLatestJobNumber(companyId);
      if (!latestAgain.ok) return NextResponse.json({ error: latestAgain.error }, { status: 500 });
      jobNumber = nextJobNumberFromLatest(latestAgain.latest?.job_number);
      continue;
    }
